package com.example.picturewatcher;

public final class Constants {
    public static final Integer ITEMS_PER_PAGE = 5;


    public static final Integer SMALL_IMAGE_W = 500, SMALL_IMAGE_H = 900;
    public static final Integer MEDIUM_IMAGE_W = 700, MEDIUM_IMAGE_H = 1100;

    public static final String
            ACCESS_KEY = "c65614934e9e6928ddf4baaf55cefa6dd50375640bcd607a3660147c0179af10",
            SECRET_KEY = "3d898acee6b7e3d3f142bd00d28f4f50659daf83de72c54e1c21fb1a0eaf6bc1";
    public static final String UNSPLASH_API_URL = "https://api.unsplash.com/";
    public static final String LOG_TAG = "PictureWatcher";
}
